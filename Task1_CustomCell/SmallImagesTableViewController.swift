...
class SmallImagesTableViewController: UITableViewController, MyTableViewCellButtonDelegate  {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.tag = indexPath.row
                             //let tapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(onCellImageTap(sender: )))
                             //cell.MyCellImageView.addGestureRecognizer(tapGestureRecognizer)
                             //cell.delegateCellImageTap = self
        ...
        if let cachedImage = cachedImages[url] {
            ...
            loadImage(url: url, for: indexPath, in: tableView)
        }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        tappedImageCellTag = indexPath.row
    ...
    override func prepare (for segue: UIStoryboardSegue, sender: Any?) {
        if (segue.identifier == "segueToLargeImage") {
            ...
            photosVC.tempURL = largePhotoURLs[tappedImageCellTag]
        }
